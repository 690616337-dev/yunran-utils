# 中国行政区划数据 - 省市级联
# 格式: {省级代码: {名称, 市级: {市级代码: 名称}}}

AREA_CODES_HIERARCHY = {
    ...
    "32": {"name": "江苏省", "cities": {
        ...
        "3209": "盐城市", "3210": "扬州市", "3211": "镇江市", "3212": "泰州市", "3213": "宿迁市"
    }},
    "33": {"name": "浙江省", "cities": {
    ...
    "41": {"name": "河南省", "cities": {
        ...
        "4113": "南阳市", "4114": "商丘市", "4115": "信阳市", "4116": "周口市", "4117": "驻马店市"
    }},
    "42": {"name": "湖北省", "cities": {
    ...
    "63": {"name": "青海省", "cities": {
        "6301": "西宁市", "6302": "海东市", "6322": "海北藏族自治州",
        "6323": "黄南藏族自治州", "6325": "海南藏族自治州", "6326": "果洛藏族自治州",
    ...
    "71": {"name": "台湾省", "cities": {"7101": "台湾省"}},
    "81": {"name": "香港特别行政区", "cities": {"8101": "香港特别行政区"}},
    "82": {"name": "澳门特别行政区", "cities": {"8201": "澳门特别行政区"}}
}

# 兼容旧代码的扁平化地区码
AREA_CODES = {code: info["name"] for code, info in AREA_CODES_HIERARCHY.items()}

# 获取所有市级代码
def get_all_city_codes():
    """获取所有市级行政区划代码"""
    cities = []
    for province_code, province_info in AREA_CODES_HIERARCHY.items():
...
# 根据市级代码获取完整地区码（6位）
def get_full_area_code(city_code: str) -> str:
    """根据市级代码获取完整的6位地区码"""
    if len(city_code) == 6:
        return city_code
    if len(city_code) == 4:
        # 市级代码，需要补充区县码
        return city_code + "01"  # 默认第一个区县
    return city_code.ljust(6, '0')[:6]
